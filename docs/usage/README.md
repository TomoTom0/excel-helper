...
- **1行目をヘッダーとして使用**: チェックONで入力データの1行目をヘッダーとして扱い、変換時にスキップします
- **カラムヘッダー**: 固定長からTSV/CSV等への変換時に先頭行に追加するヘッダーを指定（「1行目をヘッダーとして使用」がOFFの場合のみ有効）
...
- 「全て文字列として出力」オプションを有効にすると、"001"や"0123"などの先頭がゼロの数値が文字列として保持されます（ExcelでCSVを開いた際にゼロが消える問題を防げます）
